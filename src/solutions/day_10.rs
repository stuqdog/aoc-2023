...
fn get_new_direction(pipe: char, cur_direction: (i32, i32)) -> Option<(i32, i32)> {
    match (pipe, cur_direction) {
        ('|', (_, 0)) => Some(cur_direction),
        ('-', (0, _)) => Some(cur_direction),
        ('L', (1, 0)) => Some((0, 1)),
        ('L', (0, -1)) => Some((-1, 0)),
        ('J', (1, 0)) => Some((0, -1)),
        ('J', (0, 1)) => Some((-1, 0)),
...
pub fn main() {
    let input = "J.7--J-.LL77FJ-F7FLJ-7.|FFF.F7FL-7F-7.L-FF77.-F-L7.F-L--7FL7..LF|J.7FF7.FLJ.-7-F7F-J77F77.|7.777FJ7FL7F-|.FFJ7|-|7FL-.LL7F77.|J7.|JJ-7-7|-|7
...
L7LLLL--7L-JJL.LL--.|-F-L-|-7-LJJ-F.LJ--LJFJ--JL---7-LJLL-7.|JJ|.LF-FJ-LF|-7-|-J-F|-LLLL|-JLLFJLJ.L|JLJ7-F-FJ-7-J.-J-LJ7JL7-J.J7JJ|J|L-|.7-|";
    ...
    let mut the_loop: HashSet<(i32, i32)> = HashSet::new();
    ...
    for dir in [(0, 1), (0, -1), (-1, 0), (1, 0)].iter() {
        ...
        loop {
            ...
            if cur.0 < 0 || cur.1 < 0 || cur.0 >= col_len as i32 || cur.1 >= row_len as i32 {
                break;
            }
            ...
            seen.insert(cur);
            let cur_tile = map[cur.0 as usize][cur.1 as usize];
            if let Some(new_dir) = get_new_direction(cur_tile, dir) {
                dir = new_dir;
            } else {
                break;
            }
    ...
    for (r_idx, row) in map.iter().enumerate() {
        ...
        for (c_idx, ch) in row.iter().enumerate() {
            if the_loop.contains(&(r_idx as i32, c_idx as i32)) {
                if *ch == '|' || *ch == 'F' || *ch == '7' {
