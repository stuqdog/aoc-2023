...
pub fn main() {
    let input = "Game 1: 3 blue, 7 green, 10 red; 4 green, 4 red; 1 green, 7 blue, 5 red; 8 blue, 10 red; 7 blue, 19 red, 1 green
...
Game 100: 9 green, 7 blue; 1 green, 3 red, 4 blue; 15 red, 9 green; 3 blue, 6 red, 13 green; 2 red, 11 blue, 12 green";
    ...
    let maxes: HashMap<&str, i32> = [("red", 12), ("green", 13), ("blue", 14)].into();
    let mut sum = 0;
    let mut power = 0;
    for game in input.lines() {
        let mut mins: HashMap<&str, i32> = [("red", 0), ("green", 0), ("blue", 0)].into();
        let mut should_score = true;
        let (game_num, inputs) = game.split_once(": ").unwrap();
        ...
        if should_score {
            let game_num = game_num.strip_prefix("Game ").unwrap().parse::<i32>().unwrap();
            sum += game_num;
        }
    }
