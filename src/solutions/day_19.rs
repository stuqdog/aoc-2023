...
impl RuleType {
    fn of_string(s: &str) -> Self {
        if let Some((comp, remainder)) = s.split_once('>') {
...
impl InputRange {
    fn new(x: (i64, i64), m: (i64, i64), a: (i64, i64), s: (i64, i64)) -> Self {
        Self { x, m, a, s }
    }
}
...
pub fn main() {
    let (rules, inputs) = "btl{m>2179:hzd,a>965:js,x<3167:bc,crx}
...
{x=282,m=69,a=3507,s=119}"
    ...
    let rules =
        rules
            .lines()
            .into_iter()
            .map(|r| Rule::of_string(r))
            .fold(HashMap::new(), |mut acc, r| {
                acc.insert(r.name.clone(), r);
                acc
            });

    let inputs = inputs
        .lines()
        .map(|s| Input::of_string(s))
        .collect::<Vec<Input>>();
            ...
            for rule in &cur_rule.rules {
                ...
                if let Some(s) = target {
                    if s == "A" {
                        ...
                        p1 += input.x + input.m + input.a + input.s;
                    } else if s == "R" {
                        done = true;
                    } else {
                        cur_rule = rules.get(s).unwrap();
                    }
                    break;
                }
            }
